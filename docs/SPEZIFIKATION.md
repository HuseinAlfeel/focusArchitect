...
Bei 9, 8, 7 … 1 je ein Klopf-Ton, bei 0 ein klares akustisches Signal, sonst endet die Pause komplett unbemerkt, wenn man nicht gerade auf den Bildschirm schaut (Änderung 09.08.). Der Countdown prüft zusätzlich bei jedem `visibilitychange` sofort nach (Änderung 26.08.), statt nur auf den nächsten 200ms-Tick zu warten, sonst kann ein gedrosselter Hintergrund-Tab das enge Ein-Sekunden-Fenster einzelner Klopftöne verpassen, während das robustere Endsignal trotzdem ankommt. Die Klopftöne waren anfangs deutlich leiser als das Endsignal, dadurch kaum wahrnehmbar (26.08., 17.09. und erneut im Probelauf am 22.09. kam nur das Endsignal an). **Ursache gefunden am 23.09.:** Am 17.09. wurde nur die übergebene Intensität auf 0.9 angehoben und daraus geschlossen, beide lägen gleich laut - gemessen war das Endsignal trotzdem 9,9 dB lauter, weil es aus zwei Tönen besteht und mit einem größeren Faktor rechnet. Dazu lag der Klopfton bei 330 Hz, was Laptop-Lautsprecher kaum noch abstrahlen. Jetzt 600 Hz und Pegel 0,169 gegen 0,330 beim Endsignal, also rund 6 dB darunter: hörbar als Vorlauf, aber nicht als Schlusspunkt. Gemessen, nicht geschätzt.
...
**Zwei Korrekturen am 23.09., aus dem zweiten Probelauf:** Die Latenz wird jetzt aus `clientAt` gerechnet statt aus `at`, und die beiden Zeitstempelspalten `nudgeStage1At`/`firstTabVisibleAfterNudge` zeigen ebenfalls Browserzeit. Grund: Die Latenz ist eine im Browser erlebte Dauer, `at` ist dagegen der Zeitpunkt des Eintreffens beim Server und hängt am Stapelversand. Im Probelauf lag `NUDGE_STAGE_1` sechs Sekunden gestapelt, `TAB_VISIBLE` ging sofort raus - aus Serverzeit ergaben sich 82 Sekunden, aus Browserzeit 89. Die Uhren laufen zudem auseinander: im zweiten Lauf ging der Browser rund 0,75 Sekunden vor, `at` lag also sogar **vor** `clientAt`. Jetzt ist die Latenz exakt die Differenz der beiden angezeigten Spalten.

**Limitation, nicht behebbar: verspäteter Hinweis im Hintergrund-Tab.** Browser drosseln Zeitgeber in nicht sichtbaren Tabs auf etwa einen Aufruf pro Minute. Liegt der Tab im Hintergrund, wird der Übergang auf eine neue Hinweisstufe erst beim nächsten erlaubten Tick bemerkt - im zweiten Probelauf 46 Sekunden nach dem rechnerischen Rundenende, der Ton erklang entsprechend später. Der `visibilitychange`-Listener in `useNudgeStage` holt das beim Zurückkommen sofort nach, die sichtbare Erfahrung stimmt also. **Die Kernkennzahlen sind nicht betroffen:** `reactionSecondsAfterEnd` und `reactionSecondsIntoWork` werden im Moment der Reaktion gegen den gespeicherten Zielzeitpunkt gerechnet (Regel 1) und stimmen auf die Sekunde - im Probelauf nachgerechnet: 515 von 515. Betroffen sind nur der Zeitstempel von `NUDGE_STAGE_*` und die daraus abgeleitete Latenz. Gehört in die Limitationen der Arbeit.
